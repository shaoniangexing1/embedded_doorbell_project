...
void (*sound_callback)(void *arg, void *data, size_t len);
void *sound_callback_arg;
...
static void doorbell_websocket_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case WEBSOCKET_EVENT_DATA:
        if (client==websocket_client_image||data->op_code!=0x02)
        {
            //如果是图片或不是二进制文件则不处理
            break;
        }
        ...
        if (sound_callback)
        {
            sound_callback(sound_callback_arg,data->data_ptr,data->data_len);
        }
...
void doorbell_wsclient_init(void)
{
    ...
    esp_websocket_client_config_t websocket_cfg = {
        .uri = "ws://" MQTT_SERVER_IP ":" WS_SOUND_PATH,
    };
    ...
    esp_websocket_client_config_t websocket_cfg_image = {
        .uri = "ws://" MQTT_SERVER_IP ":" WS_IMAGE_PATH,
    };
    ...
    // 注册事假回调
    // 音频事件回调
    esp_websocket_register_events(websocket_client_sound, WEBSOCKET_EVENT_ANY, doorbell_websocket_event_handler, (void *)client);
    // 图片事件回调
    esp_websocket_register_events(websocket_client_image, WEBSOCKET_EVENT_ANY, doorbell_websocket_event_handler, (void *)client);
}
...
{
    ESP_ERROR_CHECK( esp_websocket_client_stop(websocket_client_sound, portMAX_DELAY));
    ESP_ERROR_CHECK( esp_websocket_client_stop(websocket_client_image, portMAX_DELAY));
}
...
{
    esp_websocket_client_send_text(websocket_client_image, image_message, len, portMAX_DELAY) ;
}

void doorbell_wsclient_send_message_sound(void *sound_message, size_t len)
{
    esp_websocket_client_send_text(websocket_client_sound, sound_message, len, portMAX_DELAY) ;
}
